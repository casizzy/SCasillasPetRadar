...
export const generateMatchEmailTemplate = (
  foundPet: FoundPet,
  lostPet: LostPet & { distance?: number },
): string => {
  const [foundLng, foundLat] = foundPet.location.coordinates;
  const [lostLng, lostLat] = lostPet.location.coordinates;

  const mapUrl = buildMapboxUrl(lostLat, lostLng, foundLat, foundLng);
  ...
  const mapSection = mapUrl
    ...
    : `<p><strong>Coordenadas donde se encontró:</strong>
         Lat: ${foundLat}, Lng: ${foundLng}</p>
       <p><strong>Coordenadas donde se perdió:</strong>
         Lat: ${lostLat}, Lng: ${lostLng}</p>`;
